...
class oneStock_FTSE100_PL(Base):
    __tablename__ = 'oneStock_FTSE100_PL'
...
@app.route('/lstock/v1')
def lsVali():
    return render_template('lstock.html')


@app.route('/lstock/count')
def lsdata_count():
    num = db.session.query(oneStock_FTSE100_PL.profilo_PL_R).count()
...
# 从数据库中一次全部查询展示成功
# 数据接口,API
@app.route('/lstock', methods=['GET'])
def lsdata():
    big_list = []
    pls = db.session.query(oneStock_FTSE100_PL.profilo_PL_R).all()  # [[0.19],[0.19]] 需要两层遍历来做一个大列表
    for item in pls:
         for i in item:
             num_int = float(i)
...
@app.route('/lstock/ajax')
def asdata_ajax():
    return render_template('lstock_ajax.html')
