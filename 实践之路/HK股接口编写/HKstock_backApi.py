...
class MHI_OneStock_PL(Base):
    __tablename__ = 'MHI_OneStock_PL'
...
# 使用接口后的渲染接口
# 这还只是一个静态的页面,不会自助
@app.route('/hkstock/v1')
def lsVali():
    return render_template('hkstock.html')


@app.route('/hkstock/count')
def lsdata_count():
    num = db.session.query(MHI_OneStock_PL.profilo_PL_R).count()
...
# 从数据库中一次全部查询展示成功
# 数据接口,API
@app.route('/hkstock', methods=['GET'])
def lsdata():
    big_list = []
    pls = db.session.query(MHI_OneStock_PL.profilo_PL_R).all()  # [[0.19],[0.19]] 需要两层遍历来做一个大列表
    for item in pls:
         for i in item:
             num_int = float(i)
...
@app.route('/hkstock/ajax')
def asdata_ajax():
    return render_template('hkstock_ajax.html')
