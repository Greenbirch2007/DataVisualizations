...
class J225_OneStock_PL(Base):
    __tablename__ = 'J225_OneStock_PL'
...
@app.route('/jstock/count')
def lsdata_count():
    num = db.session.query(J225_OneStock_PL.profilo_PL_R).count()
...
def lsdata():
    big_list = []
    pls = db.session.query(J225_OneStock_PL.profilo_PL_R).all()  # [[0.19],[0.19]] 需要两层遍历来做一个大列表
    for item in pls:
         for i in item:
             num_int = float(i)
...
@app.route('/jstock/ajax')
def asdata_ajax():
    return render_template('jstock_ajax.html')
